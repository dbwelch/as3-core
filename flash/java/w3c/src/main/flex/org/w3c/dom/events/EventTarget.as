package org.w3c.dom.events
{
	...
	*/
	public interface EventTarget
	{
		...
		* 	processing an event, it will not be triggered by the current
		...
		* 	wishes to initiate capture. After initiating capture, all events
		* 	of the specified type will be dispatched to the registered EventListener
		...
		* 	registered as a capturing listener or not. If a listener was registered twice,
		* 	one with capture and one without, each must be removed separately. Removal of
		...
		* 	event model. Events dispatched in this manner will have the same capturing
		* 	and bubbling behavior as events dispatched directly by the implementation.
		* 	The target of the event is the EventTarget on which dispatchEvent is called.
		* 
		* 	@param event Specifies the event type, behavior, and contextual information
		* 	to be used in processing the event.
		* 
		* 	@throws EventException UNSPECIFIED_EVENT_TYPE_ERR: Raised if the Event's
		* 	type was not specified by initializing the event before dispatchEvent was
		* 	called. Specification of the Event's type as null or an empty string will
